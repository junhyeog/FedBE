...
def args_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="number of trans data to use in the server: -1 for using all - num_data in users.",
    )
    ...
    parser.add_argument(
        "--max_pool", type=str, default="True", help="Whether use max pooling rather than strided convolutions"
    )
    ...
    if args.update == "FedAvg":
        args.use_SWA = False
    if args.teacher_type != "SWAG":
        args.dont_add_fedavg = True
