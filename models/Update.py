                ...
                if self.args.verbose and batch_idx % 10 == 0:
                    print(
                        "Update Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}".format(
                            ...
                            100.0 * batch_idx / len(self.ldr_train),
                            loss.item(),
                        )
                    )
...
class ServerUpdate(object):
    ...
    def record_teacher(self, ldr_train, net, teachers, global_ep, log_dir=None, probe=True, resample=False):
        ...
        cnt = 0
        for batch_idx, (images, labels) in enumerate(self.ldr_train):
            logits, batch_entropy = self.get_ensemble_logits(
                teachers, images.cuda(), method=self.args.logit_method, global_ep=global_ep
            )
        ...
        if not probe:
            ...
            plt.title(
                "Round %d, correct teacher/per sample %.2f, upperbound correct %.1f percentage"
                % (global_ep, teacher_per_sample, has_correct_teacher_ratio * 100.0)
            )
            plt.legend(loc="best")
