...
if __name__ == "__main__":
    ...
    if args.dataset == "mnist":
        ...
        dataset_eval = datasets.CIFAR10(
            "./data/cifar", train=True, transform=transform_val, target_transform=None, download=True
        )
        if args.iid:
            dict_users, server_id = cifar_iid(dataset_train, args.num_users, num_data=args.num_data)
        else:
            dict_users, server_id, cnts_dict = cifar_noniid(
                dataset_train, args.num_users, num_data=args.num_data, method=args.split_method
            )
    else:
    ...
    def put_log(logger, net_glob, tag, iters=-1):
        [acc_train, loss_train], [acc_test, loss_test], [acc_val, loss_val] = eval(
            net_glob, tag=tag, server_id=server_id
        )
    ...
    for iters in range(args.rounds):
        ...
        for i in range(0, m, num_threads):
            ...
            for idx in idxs_users[i : i + num_threads]:
                p = mp.Process(
                    target=cliet_train,
                    args=(q, idx % (args.num_gpu), copy.deepcopy(net_glob), iters, idx, server_id, generator),
                )
                p.start()
                processes.append(p)
